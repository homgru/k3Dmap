...
const LIMIT=1800,STEP=0.0000015;
...
export function addTrees(map){
 ...
 const enabled=()=>document.getElementById('landcover').checked&&map.getZoom()>=14;
 ...
 for(const c of candidates){if(count>=LIMIT)break;const ll=new maplibregl.MercatorCoordinate(c.mx,c.my).toLngLat();const screen=map.project(ll);if(screen.x< -80||screen.y< -80||screen.x>canvas.clientWidth+80||screen.y>canvas.clientHeight+80)continue;
 ...
 const h=9+noise(c.x+91,c.y)*7;v.set((c.mx-origin.x)/scale,(c.my-origin.y)/scale,altitude);size.set(1,1,h/12);matrix.compose(v,q,size);trunk.setMatrixAt(count,matrix);crown.setMatrixAt(count,matrix);crown.setColorAt(count,new THREE.Color().setHSL(.29+noise(c.x,c.y)*.045,.28,.28+noise(c.y,c.x)*.10));count++;
 }
 ...
 map.on('moveend',schedule);map.on('sourcedata',onSource);map.on('terrain',schedule);document.getElementById('landcover').addEventListener('change',schedule);
